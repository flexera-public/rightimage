...
Scenario: PHP server test
  ...
  When I stop the servers
  Then the "App Server" servers become stopped

  When I find the "App Server" servers

  When I start the servers
  Then the "App Server" servers become operational
  
  Given A deployment with started servers
  When I find the "App Server" servers
  Given I am testing the "App Server"
  Given with a known OS
  And I am using port "8000"
